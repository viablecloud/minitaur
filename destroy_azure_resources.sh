...
for subscription in "${subscriptions[@]}"; do
    ...
    destroy_specific_rgs() {
        ...
        for rg in "${specific_rgs[@]}"; do
            echo "Attempting to delete resource group: $rg in subscription: $subscription"
            # Capture the error output and explicitly specify subscription
            if output=$(az group delete --name "$rg" --subscription "$subscription" --yes --no-wait 2>&1); then
    ...
    # Final cleanup - attempt to delete all remaining resource groups
    echo -e "\n=== Final Cleanup - Deleting Remaining Resource Groups ==="
    # Get the list of remaining resource groups directly into an array
    remaining_rgs=()
    while IFS= read -r rg; do
        if [ -n "$rg" ]; then  # Only add non-empty lines
            remaining_rgs+=("$rg")
        fi
    done < <(az group list --subscription "$subscription" --query '[].name' -o tsv)

    if [ ${#remaining_rgs[@]} -eq 0 ]; then
        echo "No remaining resource groups found"
    else
        echo "Found ${#remaining_rgs[@]} remaining resource groups"
        echo "Resource groups to be deleted:"
        printf '%s\n' "${remaining_rgs[@]}"
        ...
        for rg in "${remaining_rgs[@]}"; do
            ...
            echo "Attempting to delete remaining resource group: $rg in subscription: $subscription"
            if output=$(az group delete --name "$rg" --subscription "$subscription" --yes --no-wait 2>&1); then
                DESTROYED_RESOURCES["$rg,<final-cleanup>"]=true
                ((total_destroyed++))
                echo "Successfully initiated deletion of resource group: $rg"
            else
                DESTROYED_RESOURCES["$rg,<final-cleanup>"]=false
                FAILED_RESOURCES+=("$rg")
                ((total_failed++))
                echo "Failed to delete resource group: $rg"
                echo "Error output: $output"
