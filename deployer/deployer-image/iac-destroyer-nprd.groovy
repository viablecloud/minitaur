pipelineJob('Infrastructure/NonProduction/landing-zone-destroyer-nprd') {
    definition {
        cps {
            sandbox(true)
            script('''
...
*/
        ...
        stage("Validating Current Infrastructure") {
            steps {
                script {
                    sh 'chmod +x ./test.sh'
                    sh './test.sh'
        ...
        stage("Destroying Current Infrastructure") {
            steps {
                script {
                    sh 'pwd'
                    sh 'ls -l'
                    sh './destroy_azure_resources.sh exclude.txt'
                }           
            }               
        }
