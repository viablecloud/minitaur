pipelineJob('Infrastructure/NonProduction/landing-zone-nprd') {
    definition {
        cps {
            sandbox(true)
            script('''
...
*/
        ...
        stage("Validating Infrastructure State") {
            steps {
                script {
                    sh 'chmod +x ./test.sh'
                    sh './test.sh'
        ...
        stage("Deploying Infrastructure State") {
            steps {
                script {
        ...
        stage("Kubernetes Validation Checks") {
            steps {
                script {
                    sh 'chmod +x ./check-aks-cert-readiness.sh'
                    sh './check-aks-cert-readiness.sh'
                }
            }
        }
